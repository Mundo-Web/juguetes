    ...
    @foreach ($datosgenerales as $dato)
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-12  md:justify-center w-11/12 mx-auto py-16 ">
                    ...
                    <ul>
                        @if (!empty($dato->address) || !empty($dato->district)  || !empty($dato->country) )
                            <li class="flex flex-row gap-2">
                                <img src="{{ asset('/images/svg/jl_map.svg') }}" alt="Address Icon" />
                                {{ $dato->address }} - {{ $dato->district }} - {{ $dato->country }}
                            </li>
                        @endif
                        @if (!empty($dato->email))
                            <li class="flex flex-row gap-2">
                                <img src="{{ asset('/images/svg/jl_mail.svg') }}" alt="Email Icon" />
                                {{ $dato->email }}
                            </li>
                        @endif
                        @if (!empty($dato->whatsapp))
                            <li class="flex flex-row gap-2">
                                <img src="{{ asset('/images/svg/jl_whatsapp.svg') }}" alt="WhatsApp Icon" />
                                {{ $dato->whatsapp }}
                            </li>
                        @endif
                    </ul>
            ...
            <div class="lg:col-span-3 w-full  flex flex-col gap-5">
                ...
                <nav>
                    <ul class="flex flex-col gap-3 text-base font-normal">
                        <li class="flex flex-row gap-2"><a href="{{ route('politicas_dev') }}"> Políticas de privacidad
                            </a></li>
                        <li class="flex flex-row gap-2"><a href="{{ route('politicas_dev') }}"> Políticas de envío </a>
                        </li>
                        <li class="flex flex-row gap-2"><a href="{{ route('politicas_dev') }}"> Políticas de devolución
                                y cambio </a></li>
                    </ul>
                </nav>
            ...
            <div class="lg:col-span-2 w-full flex flex-col gap-5">
                ...
                <nav>
                    <ul class="flex flex-col gap-3 text-base font-normal">
                        <li class="flex flex-row gap-2"><a href="{{ route('index') }}"> Inicio </a></li>
                        <li class="flex flex-row gap-2"><a href="{{ route('catalogo.all') }}"> Productos </a></li>
                        <li class="flex flex-row gap-2"><a href="{{ route('blog', 0) }}">Blog </a></li>
                        <li class="flex flex-row gap-2"><a href="{{ route('contacto') }}">Contacto</a></li>
                    </ul>
                </nav>
            </div>
            ...
            <div class="lg:col-span-3 w-full  flex flex-col gap-5">
                ...
                <div class="flex flex-col gap-2">
                    <form action="" id="footerFormulario"
                        ...
                        @csrf
                        <div class="w-full">
                            <input required name="email" type="email" id="emailFooter"
                                class="ring-0 focus:ring-0 border-transparent focus:border-transparent bg-white px-5 py-3 rounded-xl w-full text-colorJL placeholder:text-colorJL"
        ...
        <div class="mt-5 flex flex-col md:flex-row md:justify-between md:items-center gap-5 w-11/12 mx-auto">
            <div class="flex flex-col md:flex-row gap-2">
                <p class="font-normal text-[13px]">
                    Copyright &copy; <a target="_blank" href="http://mundoweb.pe/" class="font-bold">2024 Mundo Web</a>.
